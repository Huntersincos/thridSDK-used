...
/*
 ...
 sqlite的WAL机制:
 我们在写数据时，并不是直接操作数据库文件，而是操作一个WAL文件，这样当我们插入、或者删除数据时，只是写一条操作语句到WAL文件，当WAL文件达到一定数量级（默认是1000page）时，sqlite就会把数据同步到数据库文件中。其带来的不足之处是读数据时，可能要同时考虑到WAL文件与数据库文件，所以读数据性能会有一定下降。但这个对于移动端的数据量来说表现得并不明显。所以数据库在删除数据后，数据库文件大小有时候反而会增大。但并不会无限制地增大，当增长到一定数量时，其就会变小。
 */
